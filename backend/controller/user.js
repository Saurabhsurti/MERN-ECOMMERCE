...
//     const existAvatarPath = path.join("uploads", existsUser.avatar.url); // Use existsUser.avatar.url here
...
//       avatar: {
//         public_id: "avatar", // Assuming this is a static value
//         url: fileUrl,
//       },
//     });
...
router.put("/update-avatar", isAuthenticated, upload.single("image"), catchAsyncErrors(async(req, res, next) => {
  try {
    ...
    const user = await User.findByIdAndUpdate(req.user.id, {avatar:{url: fileUrl, // Set the URL based on the uploaded file's path
    public_id: "avatar"}})
