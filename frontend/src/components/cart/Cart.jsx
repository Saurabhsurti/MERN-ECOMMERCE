    ...
    <div className="border-b p-4">
      <div className="w-full flex items-center">
        <div>
          ...
          <div className='bg-[#a7abb14f] rounded-full w-[25] h-[25px] flex items-center justify-center cursor-pointer' onClick={() => decrement(data)}>
            <HiOutlineMinus size={16} color="#7d879c" />
          </div>
        </div>
        <img src={`${backend_url}${data?.images[0]}`} alt="Sorry" className='w-[130px] h-min ml-2 mr-2 rounded-[5px]' />
        <div className='pl-[5px]'>
          <h1>{data.name}</h1>
          <h4 className='font-[400] text-[15px] text-[#000000]'>${data.discountPrice} * {value}</h4>
          <h4 className='font-[600] text-[17px] pt-[3px] text-[#d02222] font-Roboto'>
            US${totalPrice}
          </h4>
        </div>
        <RxCross1 size={40} className='cursor-pointer' onClick={() => removeFromCartHandler(data)} />
      </div>
    </div>
  );
};
